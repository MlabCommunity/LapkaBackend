...
public record GetAllLikedShelterAdvertisementsQuery(double Longitude, double Latitude, GetAllShelterAdvertisementsRequest Request,PaginationDto Pagination, Guid UserId) : IRequest<ShelterPetAdvertisementDtoPagedResult>;
        ...
        public async Task<ShelterPetAdvertisementDtoPagedResult> Handle(GetAllLikedShelterAdvertisementsQuery query, CancellationToken cancellationToken)
        {
            ...
            foreach (var shelter in shelters)
            {
                ...
                petsFromShelter = petsFromShelter.Where(pet => 
                    pet.Name.Contains(query.Pagination.SearchText) 
                    || shelter.OrganizationName.Contains(query.Pagination.SearchText) 
                    || shelter.City.Contains(query.Pagination.SearchText)).ToList();
